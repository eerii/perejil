...
d = pd.read_csv("pasabaixospython.csv", sep=',', decimal='.')

f = d["f"].to_numpy()
o = d["th"].to_numpy()

def arctanf(x,a,b,c):
    return a + b * np.arctan(c / np.abs(x))
popt, pcov = curve_fit(arctanf, f, o)

print(popt)

xs = np.logspace(0., 5., 500)
ys = (arctanf(xs, *popt))
...
guardar("pasabaixos", "$\\log(f(Hz))$", "$\\theta(rad)$", leg=False)
...
x1 = np.logspace(0., 5., 500)
y1 = np.ones(x1.shape[0])
plt.plot(x1, y1,'--', color="sandybrown")

x2 = np.logspace(2., 5., 500)
y2 = 1 - 0.0000100197 * np.exp(-x2)
#plt.plot(x2, y2,'--', color="sandybrown")

plt.scatter(197, 1, linewidth=0.5, s=20, color="tomato")

guardar("pasabaixos2", "$\\log(f(Hz))$", "$V_2/V_1$", leg=False)